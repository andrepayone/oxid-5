...
class fcpoRequest extends oxSuperCfg {
    ...
    protected function setAuthorizationParameters($oOrder, $oUser, $aDynvalue, $sRefNr, $blIsPreauthorization = false) {
       ...
       if ($oOrder->oxorder__oxdellname->value != '') {
        ...
        $blAddProductInfo = (
            $oOrder->isDetailedProductInfoNeeded() || $this->getConfig()->getConfigParam('blFCPOSendArticlelist') === true);
    ...
    public function addProductInfo($oOrder, $aPositions = false, $blDebit = false) {
        ...
        if ($aPositions === false || $blFirstCapture === true || $blDebit === true) {
            ...
            if ($oOrder->oxorder__oxwrapcost->value != 0 && ($aPositions === false || ($blDebit === false || array_key_exists('oxwrapcost', $aPositions) !== false))) {
                ...
                // Workaround for wrong vat: oxid saves 18.95... in Db so we use order vat here instead
            ...
            } else if ($oOrder->oxorder__oxvoucherdiscount->value != 0 && ($aPositions === false || ($blDebit === false || array_key_exists('oxvoucherdiscount', $aPositions) !== false))) {
    ...
    protected function _fcpoAddBasketItemsFromSession($sDeliverySetId = false)
    {
        ...
        if ($fPaymentCosts != 0) {
            $sPayDesc = $fPaymentCosts > 0 ? $oLang->translateString('FCPO_SURCHARGE', null, false) : $oLang->translateString('FCPO_DEDUCTION', null, false);
            $sPayDesc .= ' ' . str_replace(':', '', $oLang->translateString('FCPO_PAYMENTTYPE', null, false));
            $this->addParameter('it[' . $iIndex . ']', 'handling');
            $this->addParameter('id[' . $iIndex . ']', 'payment');
            $this->addParameter('pr[' . $iIndex . ']', $this->_fcpoGetCentPrice($fPaymentCosts));
            $this->addParameter('no[' . $iIndex . ']', '1');
            $this->addParameter('de[' . $iIndex . ']', $sPayDesc);
            $this->addParameter('va[' . $iIndex . ']', $this->_fcpoFetchVatCostsFromBasket($oBasket, 'oxpayment') * 100);
            $iIndex++;
        }
    ...
    /**
     * Returns vat of given basket object
     *
     * @param $oBasket
     * @param $costType // e.g. oxdelivery, oxwrapping, oxgiftcard
     * @return mixed float|string
    ...
    protected function addAddressParamsByUser($oUser, $isKlarna = false) {
        ...
        if ($blHasShipping) {
            ...
            $this->addParameter('shipping_firstname', $oUser->oxuser__oxfname->value);
            $this->addParameter('shipping_lastname', $oUser->oxuser__oxlname->value);
            $this->addParameter('shipping_street', trim($oUser->oxuser__oxstreet->rawValue . ' ' . $oUser->oxuser__oxstreetnr->value));
